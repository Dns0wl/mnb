...
class HW_DNS_Manual_Admin
{
    ...
    public function handle_create_manual()
    {
        ...
        if (!isset($_POST['_hw_dns_manual_nonce']) || !wp_verify_nonce($_POST['_hw_dns_manual_nonce'], 'hw_dns_manual_nonce')) {
            wp_safe_redirect(add_query_arg('message', 'error', menu_page_url('hw-dns-manual', false)));
            exit;
        }
        ...
        if ($date_mode === 'now') {
            ...
            $purchase_date = $timestamp ? date('d/m/y', $timestamp) : $raw_date;
        }
        ...
        wp_safe_redirect(add_query_arg('message', 'created', menu_page_url('hw-dns-manual', false)));
        exit;
    }
